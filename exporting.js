(function (l) {
	...
	(function (f) {
		...
		t(l.lang, {
			...
			downloadSVG: "Download SVG vector Bild",
			contextButtonTitle: "Diagramm kontext menu"
		});
		l.navigation = {
			buttonOptions: {
				theme: {},
		...
		t(A.prototype, {
			sanitizeSVG: function (a, b) {
				...
				a = a.replace(/zIndex="[^"]+"/g, "").replace(/isShadow="[^"]+"/g, "").replace(/symbolName="[^"]+"/g, "").replace(/jQuery[0-9]+="[^"]+"/g, "").replace(/url\(("|")(\S+)("|")\)/g, "url($2)").replace(/url\([^#]+#/g, "url(#").replace(/<svg /, '\x3csvg xmlns:xlink\x3d"http://www.w3.org/1999/xlink" ').replace(/ (NS[0-9]+\:)?href=/g,
					" xlink:href\x3d").replace(/\n/, " ").replace(/<\/svg>.*?$/, "\x3c/svg\x3e").replace(/(fill|stroke)="rgba\(([ 0-9]+,[ 0-9]+,[ 0-9]+),([ 0-9\.]+)\)"/g, '$1\x3d"rgb($2)" $1-opacity\x3d"$3"').replace(/ /g, "\u00a0").replace(/­/g, "\u00ad");
